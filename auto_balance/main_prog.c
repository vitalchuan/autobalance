...
void loop(void)
{
    start_motor();
    float angleAx, AX1;
    static char value_a[10];
    static char value_b[10];

    memset(value_a, 0x00, 10);
    memset(value_b, 0x00, 10);
    ...
    itoa(enleft, value_a, 10);
    itoa(enright, value_b, 10);

//    send_string_pc(value_a, 10);
//    send_string_pc("\t",2);
//    send_string_pc(value_b, 10);
//    send_string_pc("\n",2);
    ...
    AX1=Yijiehubulvbo(angleAx,-mpu_data.GYR_X,dt);
...
//    set_motor_1_direction(1);
//    set_motor_2_direction(1);
    if(control_motor()==1)
    {
